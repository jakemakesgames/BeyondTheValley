...
namespace Strata
{
    [CreateAssetMenu(menuName = "Strata/Generators/Generator Flood Fill")]
    ...
    public class GeneratorFloodFill : Generator
    {
        ...
        private void BoardFloodFill(BoardGenerator boardGenerator, GridPosition gridPosition, char targetChar, char replacementChar)
        {
            ...
            while (gridPositions.Count != 0)
            {
                ...
                while (y1 < boardGenerator.profile.boardVerticalSize && boardGenerator.boardGridAsCharacters[temp.x, y1] == targetChar)
                {
                    ...
                    if (!spanRight && temp.x < boardGenerator.profile.boardHorizontalSize- 1 && boardGenerator.boardGridAsCharacters[temp.x + 1, y1] == targetChar)
                    {
                        gridPositions.Push(new GridPosition(temp.x + 1, y1));
                        spanRight = true;
                    }
                    else if (spanRight && temp.x < boardGenerator.profile.boardHorizontalSize - 1 && boardGenerator.boardGridAsCharacters[temp.x + 1, y1] != targetChar)
                    {
                        spanRight = false;
                    }
                    y1++;
